<?php
  ...
  if(isset($_FILES['fileCalciatori'])) {
    ...
    if ($file_error === UPLOAD_ERR_OK) {
      ...
      header("Location: ../visualizzaCalciatori?page=1.php");
      exit;
    } else {
      // Gestione degli errori di upload
      switch ($file_error) {
        case UPLOAD_ERR_INI_SIZE:
          header("Location: ../inserisciCalciatori.php?check=Il file
          caricato supera la dimensione massima consentita.");
          exit;
        case UPLOAD_ERR_FORM_SIZE:
          header("Location: ../inserisciCalciatori.php?check=Il file
          caricato supera la dimensione massima consentita nel form HTML.");
          exit;
        case UPLOAD_ERR_PARTIAL:
          header("Location: ../inserisciCalciatori.php?check=Il file è
          stato caricato solo parzialmente.");
          exit;
        case UPLOAD_ERR_NO_FILE:
          header("Location: ../inserisciCalciatori.php?check=Nessun file è stato caricato.");
          exit;
        case UPLOAD_ERR_NO_TMP_DIR:
          header("Location: ../inserisciCalciatori.php?check=Manca una
          cartella temporanea.");
          exit;
        case UPLOAD_ERR_CANT_WRITE:
          header("Location: ../inserisciCalciatori.php?check=Impossibile
          scrivere il file sul disco.");
          exit;
        case UPLOAD_ERR_EXTENSION:
          header("Location: ../inserisciCalciatori.php?check=Un'estensione
          PHP ha bloccato l'upload del file.");
          exit;
        default:
          header("Location: ../inserisciCalciatori.php?check=Si è
          verificato un errore durante l'upload del file.");
          exit;
      }
    }
  }
